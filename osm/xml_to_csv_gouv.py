...
node_tags= {':ID','osmId', 'lat', 'lon', 'street', 'loc-name', 'district', 'city', 'city-surface',
            'city-centre', 'city-population', 'dpt', 'reg'}
...
with io.open(dir_name +'/nodes.csv', 'w', encoding="utf-8") as f:
    ...
    for node in root.findall('node'):
        ...
        if feat:
            ...
            dct['city']=addr['city']
            print( addr['citycode'])
            paramCity = {"code": addr['citycode'], 'fields':'surface,centre,population'}
            responseCity = requests.get("https://geo.api.gouv.fr/communes/", params=paramCity)
            print(responseCity.json())
            if responseCity.json() :
                responseCity = responseCity.json()[0]
                dct['city-surface'] = responseCity['surface']
                dct['city-centre'] = responseCity['centre']['coordinates']
                dct['city-population'] = responseCity['population']
            else:
                print('xx')
